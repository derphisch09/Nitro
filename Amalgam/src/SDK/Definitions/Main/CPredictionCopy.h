...
public:
	CPredictionCopy(int type, void* dest, bool dest_packed, void const* src, bool src_packed,
		bool counterrors = false, bool reporterrors = false, bool performcopy = true, bool describefields = false, FN_FIELD_COMPARE func = nullptr)
	{
		m_nType = type;
		m_pDest = dest;
		m_pSrc = src;
	...
	SIGNATURE_ARGS(TransferData, int, CPredictionCopy, (const char* operation, int entindex, datamap_t* dmap), this, operation, entindex, dmap);
};