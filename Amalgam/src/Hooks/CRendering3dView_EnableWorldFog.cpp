...
MAKE_HOOK(CRendering3dView_EnableWorldFog, S::CRendering3dView_EnableWorldFog(), void)
{
	...
	if (auto pRenderContext = I::MaterialSystem->GetRenderContext())
	{
		...
		pRenderContext->FogStart(Vars::Visuals::World::FogStart.Value);
		pRenderContext->FogEnd(Vars::Visuals::World::FogEnd.Value);
		pRenderContext->FogMaxDensity(Vars::Visuals::World::FogMaxDensity.Value);
	}
}