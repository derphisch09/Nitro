...
MAKE_HOOK(CNetChannel_SendNetMsg, S::CNetChannel_SendNetMsg(), bool, CNetChannel* pNetChan, INetMessage& msg, bool bForceReliable, bool bVoice)
{
#ifdef DEBUG_HOOKS
...
>>>>>>> Stashed changes
			...
			if (Vars::Debug::Logging.Value)
			{
				...
				{
				case FNV1A::Hash32Const("cl_interp"):
					SDK::Output("SendNetMsg", std::format("{}: {}", localCvar->Name, localCvar->Value).c_str(), { 133, 0, 255 });
				case FNV1A::Hash32Const("cl_interp_ratio"):
					SDK::Output("SendNetMsg", std::format("{}: {}", localCvar->Name, localCvar->Value).c_str(), { 133, 0, 255 });
				case FNV1A::Hash32Const("cl_interpolate"):
					SDK::Output("SendNetMsg", std::format("{}: {}", localCvar->Name, localCvar->Value).c_str(), { 133, 0, 255 });
				...
				case FNV1A::Hash32Const("cl_updaterate"):
					SDK::Output("SendNetMsg", std::format("{}: {}", localCvar->Name, localCvar->Value).c_str(), { 133, 0, 255 });
				case FNV1A::Hash32Const("cl_forwardspeed"):
					SDK::Output("SendNetMsg", std::format("{}: {}", localCvar->Name, localCvar->Value).c_str(), { 133, 0, 255 });
				case FNV1A::Hash32Const("cl_backspeed"):
					SDK::Output("SendNetMsg", std::format("{}: {}", localCvar->Name, localCvar->Value).c_str(), { 133, 0, 255 });
				case FNV1A::Hash32Const("cl_sidespeed"):
					SDK::Output("SendNetMsg", std::format("{}: {}", localCvar->Name, localCvar->Value).c_str(), { 133, 0, 255 });
				}
			}
		}
